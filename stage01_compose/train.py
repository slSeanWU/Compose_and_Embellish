...
def train(epoch, model, dloader, optim, sched):
  model.train()
  recons_loss_rec = 0.
  accum_samples = 0
  ...
  for batch_idx, batch_samples in enumerate(dloader):
    ...
    print ('-- ep {:03d} | batch {:03d}: loss = {:.4f}, step = {}, time_elapsed = {:.2f} secs'.format(
      epoch, 
      batch_idx, 
...
def validate(epoch, model, dloader, rounds=1):
  model.eval()
  recons_loss_rec = []
...
if __name__ == '__main__':
  ...
  for ep in range(start_epoch, config['training']['max_epoch']):
    recons_loss, ep_time = train(ep+1, model, dloader, optimizer, scheduler)
    ...
    if not (ep + 1) % config['training']['val_interval']:
      val_recons_losses = validate(ep+1, model, val_dloader)
